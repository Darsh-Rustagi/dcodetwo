...
export default function DashboardPage() {
  const [isMentor, setIsMentor] = useState(false); // toggle dashboard
            ...
            <ul>
              ...
              {isMentor ? (
                ...
                <>
                  ...
                  <li className="mb-2">
                    <a href="/chats" className="flex items-center gap-3 p-3 rounded-lg hover:bg-amber-200/50 font-medium text-zinc-700">
                      <MessageSquare size={20} /> Messages
                    </a>
                  </li>
                  <li className="mb-2">
        ...
        <div className="flex-1">
          <header className="bg-amber-100/80 backdrop-blur-md border-b border-amber-200/80 p-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              <button className="relative text-zinc-600 hover:text-zinc-900">
                <Bell size={24} />
              </button>
              <div className="flex items-center gap-3">
                <img
                  src={isMentor ? mentorData.avatar : studentData.avatar}
                  alt={isMentor ? mentorData.name : studentData.name}
            ...
            {mentorData.upcomingSessions.map((session) => (
              <div key={session.id} className="flex items-center justify-between p-3 bg-amber-100/70 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-zinc-600">with {session.studentName}</p>
                </div>
              ...
              <li key={activity.id} className="flex items-center gap-4 p-3 rounded-lg">
                ...
                <div className="flex-grow">
                  <p className="text-sm text-zinc-700 leading-tight">
                    <span className="font-bold text-zinc-900">{activity.from}</span>: {activity.content}
                  </p>
                </div>
            ...
            {studentData.upcomingSessions.map((session) => (
              <div key={session.id} className="flex items-center justify-between p-3 bg-amber-100/70 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold">{session.topic}</p>
                    <p className="text-sm text-zinc-600">with {session.mentorName}</p>
                  </div>
                </div>
                <p className="text-sm font-medium text-amber-800">{session.time}</p>
              </div>
            ...
            {studentData.learningProgress.map((item) => (
              <div key={item.subject}>
                ...
                <div className="w-full bg-amber-200/50 rounded-full h-2.5">
                  <div className="bg-amber-500 h-2.5 rounded-full" style={{ width: `${item.progress}%` }}></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-4">
            {studentData.recommendedMentors.map((mentor) => (
              <div key={mentor.id} className="flex items-center justify-between">
                ...
                <a href={`/mentors#${mentor.id}`} className="p-2 rounded-full hover:bg-amber-100/80">
                  <ArrowRight size={18} />
                </a>
              </div>
            ))}
