...
function StudentDashboardContent({ onProfileUpdate }) {
  ...
  const [isEditingProfile, setIsEditingProfile] = useState(false);
  const [profileForm, setProfileForm] = useState({ name: studentData.name, avatar: studentData.avatar });
  ...
  const handleSaveProfile = () => {
    ...
    setStudentData(prev => ({ ...prev, name: profileForm.name, avatar: newAvatar }));
    setIsEditingProfile(false);
  };
      ...
      <div className="lg:col-span-2 space-y-8">
        <motion.div layout className="bg-white/70 backdrop-blur-xl p-6 rounded-2xl border border-white/50 shadow-lg">
          <div className="flex justify-between items-center mb-4">
            ...
            {!isEditingProfile && <button onClick={() => { setProfileForm({ name: studentData.name, avatar: studentData.avatar }); setIsEditingProfile(true); }} className="p-2 rounded-full hover:bg-amber-100/80"><Edit size={18} /></button>}
          </div>
          <AnimatePresence mode="wait">
            {isEditingProfile ? (
              ...
              <motion.div key="view-student-profile" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-4 p-4 bg-amber-100/70 rounded-lg">
